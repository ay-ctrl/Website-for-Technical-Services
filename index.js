...
const mongoSanitize = require("express-mongo-sanitize"); // - NoSQL Injection koruması için eklendi
const rateLimit = require("express-rate-limit"); // - Genel hız sınırlama için eklendi
const app = express();
...
// - Veritabanı sorgularındaki tehlikeli karakterleri ($ gibi) temizler.
// GitHub Alert #14, #15, #16 çözümüdür.
...
// --- Güvenlik Sınırlamaları (Rate Limiting) ---

// Tüm API isteklerine genel bir sınır koyar.
const apiLimiter = rateLimit({
  windowMs: 15 * 60 * 1000, // 15 dakika
  max: 100, // Her IP adresi 15 dakikada en fazla 100 istek atabilir
  message: {
    message:
...
app.post("/api/repairRequests", async (req, res) => {
  try {
    ...
    if (!phoneRegex.test(phone?.trim())) {
      return res
        .status(400)
        .send({ message: "Geçerli bir telefon numarası girin." });
    }
    ...
    if (!adresRegex.test(adress?.trim())) {
      return res
        .status(400)
        .send({ message: "Adres geçersiz karakterler içeriyor." });
    }
    ...
    if (kilit) {
      if (kilit.length > 20) {
        return res
          .status(400)
          .send({ message: "Tuş kilidi en fazla 20 karakter olabilir." });
      }
    }
    ...
    if (!res.headersSent) {
      res
        .status(400)
        .send({ message: "Bir hata oluştu, lütfen tekrar deneyin." });
    }
  }
});
...
app.post("/api/login", loginLimiter, async (req, res) => {
  ...
  try {
    ...
    res.cookie("token", token, {
      ...
      secure: process.env.NODE_ENV === "production", // Güvenlik için düzenlendi
    ...
    try {
      ...
      if (req.file) {
        try {
          ...
          logger.info(
            `File uploaded to Google Drive, File ID: ${fileId}, IP: ${req.ip}`,
          );
        } catch (googleError) {
          logger.error(
            `Google Drive yükleme hatası: ${googleError.message}, IP: ${req.ip}`,
          );
          ...
          return res
            .status(500)
            .json({ error: "Google Drive yükleme hatası oluştu!" });
        }
      }
      ...
      logger.info(
        `New campaign uploaded successfully, Campaign ID: ${newCampaign._id}, IP: ${req.ip}`,
      );
      ...
      if (req.file) {
        // path.basename: Dizin geçişlerini (../../) temizler ve sadece dosya adını alır
        ...
        if (fs.existsSync(filePath)) {
          try {
            ...
            logger.info(`Geçici dosya güvenle silindi: ${safeFileName}`);
          } catch (err) {
            logger.error(`Dosya silme hatası: ${err.message}`);
          }
        }
      }
    }
...
app.post(
  ...
  async (req, res) => {
    try {
      ...
      await product.save();
      logger.info(
        `Product uploaded successfully, Product ID: ${product.id}, IP: ${req.ip}`,
      );
...
fs.readFile(TOKEN_PATH, (err, token) => {
  if (err) {
    ...
    logger.error(
      `Token dosyası bulunamadı veya okuma hatası oluştu: ${err.message}`,
    );
    return;
  }
