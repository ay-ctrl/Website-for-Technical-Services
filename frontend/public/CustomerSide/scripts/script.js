...
async function talepOlustur(event) {
  ...
  if (!modelRegex.test(model)) {
    alert(
      "Telefon modeli yalnızca harf, rakam, boşluk ve - _ . karakterlerini içerebilir.",
    );
    return;
  }
  ...
  if (!adresRegex.test(adres)) {
    alert(
      "Adres sadece harf, rakam, boşluk, nokta, /, -, ( ve ) karakterlerinden oluşabilir.",
    );
    return;
  }
...
function showRequestInfo() {
  const repairRequestData = JSON.parse(
    localStorage.getItem("repairRequestData"),
  );

  if (repairRequestData) {
    // Veriyi tabloya doldur
    document.getElementById("customerName").textContent =
      repairRequestData.name;
    document.getElementById("customerPhone").textContent =
      repairRequestData.phone;
    document.getElementById("customerAddress").textContent =
      repairRequestData.adress;
    document.getElementById("problemDescription").textContent =
      repairRequestData.sorunlar.join(", ");
    document.getElementById("createdAt").textContent = new Date(
      repairRequestData.createdAt,
    ).toLocaleDateString();
    document.getElementById("status").textContent =
      repairRequestData.state || "Bilinmiyor";
    document.getElementById("price").textContent =
      repairRequestData.price || "Belirlenmedi";
    // Tabloyu göster
...
async function talepSorgula() {
  ...
  try {
    const response = await fetch(
      `${window.API_URL}/api/repairRequests/search`,
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
